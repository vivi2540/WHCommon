...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        //3.添加手势
        [mainImageView addGestureRecognizer:longTap];
...
-(void)imglongTapClick:(UILongPressGestureRecognizer*)gesture
{
    if(gesture.state==UIGestureRecognizerStateBegan)
    {
        ...
        UIAlertAction *confirm = [UIAlertAction actionWithTitle:@"确认" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
            ...
            UIImage *img = [[UIImage alloc]initWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:self.model.imgUrl]]];
            // 保存图片到相册
...
//        [self.mainImageView setImageWithURL:[NSURL URLWithString:self.model.imgUrl] placeholder:self.holdImg options:0 completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, YYWebImageFromType from, YYWebImageStage stage, NSError * _Nullable error) {
...
//            }
//        }];
        [self.mainImageView sd_setImageWithURL:[NSURL URLWithString:self.model.imgUrl] placeholderImage:self.holdImg options:SDWebImageRetryFailed| SDWebImageLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
