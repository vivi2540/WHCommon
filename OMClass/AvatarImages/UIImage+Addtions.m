...
+ (NSString *)convertToBase64:(UIImage *)image{
    NSString *dataStr = @"";
    NSData *imageData = UIImageJPEGRepresentation(image, 1);
    NSUInteger  length = [imageData length]/1024;
    if (length > 1000) {
        NSData *imageData2 = UIImageJPEGRepresentation(image, 0.6);
        dataStr = [imageData2 base64EncodedStringWithOptions:0];
    }else{
        dataStr = [imageData base64EncodedStringWithOptions:0];
    }
    NSString *typeStr = [UIImage typeForImageData:imageData];
    
    NSString *urlS = @"";
    if ([typeStr isEqualToString:@"image/jpeg"]) {
        urlS =  [NSString stringWithFormat:@"data:image/jpeg;base64,%@",dataStr];
    }else{
        urlS =  [NSString stringWithFormat:@"data:image/png;base64,%@",dataStr];
    }
    return urlS;
}
