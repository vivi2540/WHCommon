...
- (void)drawRect:(CGRect)rect{
    ...
    UIBezierPath *path = [UIBezierPath bezierPathWithRect:CGRectMake(0.5, rect.size.height - 0.5, rect.size.width-1, 0.5)];
...
- (CGPoint)getRefPoint{
    ...
    if (self.orientation==down||self.orientation==right) {
        refPoint = CGPointMake(absoluteRect.origin.x + absoluteRect.size.width / 2, absoluteRect.origin.y + absoluteRect.size.height);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell.textLabel mas_updateConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(cell.imageView.mas_right).mas_offset(6);
        make.centerY.mas_equalTo(cell.mas_centerY);
...
- (void)setBorderColor:(UIColor *)borderColor{
    if (borderColor) {
      _borderColor = borderColor;
        self.contentView.layer.mask.borderWidth=0.5;
        self.contentView.layer.mask.borderColor= _borderColor.CGColor;
