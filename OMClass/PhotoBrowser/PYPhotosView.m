...
- (void)setPhotos:(NSArray<PYPhoto *> *)photos
{
    ...
    while (self.subviews.count < photoCount) { // UIImageView不够，需要创建
        ...
        photoView.DoNotEdit = self.DoNotEdit;
    ...
    CGFloat width = size.width + self.py_x > PYScreenW ? PYScreenW - self.py_x + 13 : size.width + 13;
...
- (CGSize)sizeWithPhotoCount:(NSInteger)count photosState:(NSInteger)state
{
    ...
    photosViewH = rows * self.photoHeight + (rows - 1) * self.photoMargin+15;
    if (count == 0) {
...
- (void)layoutSubviews
{
    ...
    if (self.photos.count == 1) {
        ...
        photoView.DoNotEdit = self.DoNotEdit;
        if (self.isHiddenDele) {
            photoView.deleteImageView.hidden = YES;
            photoView.DoNotEdit = YES;
        }
    ...
    for (int i = 0; i < photosCount; i++) {
        ...
        photoView.py_y = row * (self.photoHeight + self.photoMargin -3)+10;
        photoView.py_x = col * (self.photoWidth + self.photoMargin)+7;
        photoView.py_width = self.photoWidth;
        photoView.py_height = self.photoHeight;
//        NSLog(@"外面--%f,--%f",self.photoWidth,self.photoHeight);
    }
    if (self.images.count < self.imagesMaxCountWhenWillCompose && self.photosState == PYPhotosViewStateWillCompose) {
        ...
        self.addImageButton.py_y = (self.images.count / maxCol) * (self.photoHeight + self.photoMargin - 1) +10;
        self.addImageButton.py_x = (self.images.count % maxCol) * (self.photoWidth + self.photoMargin) + 10;
        self.addImageButton.py_width = self.photoWidth;
        self.addImageButton.py_height = self.photoHeight;
        if (self.images.count == 0) { // 数组为空
            self.py_size = CGSizeMake(self.addImageButton.py_size.width + 10, self.addImageButton.py_size.height + 15) ;
        }
    }
}
