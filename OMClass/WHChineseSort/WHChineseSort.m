...
//  Star
//
//  Created by 何文虎 on 2017/3/15.
//  Copyright © 2017年 juLong. All rights reserved.
//

#import "WHChineseSort.h"

//特殊字符统一合并后的  显示字符
#define AbnormalLetter @"#"

@interface WHChineseSort()
...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr{
    ...
    {
        NSString *pinyin = [((WHChineseSort*)object).pinYin substringToIndex:1];
        //不同
...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr{
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((WHChineseSort*)object).pinYin substringToIndex:1];
        NSString *string = ((WHChineseSort*)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr{
    ...
    {
        WHChineseSort *chineseString = [[WHChineseSort alloc]init];
        ...
        chineseString.string = [WHChineseSort RemoveSpecialCharacter:chineseString.string];
...
+(NSMutableArray*)sortObjectArray:(NSArray*)objectArray Key:(NSString *)key{
    ...
    NSString *tempString;
    //拼音分组
    for (WHChineseSort* object in tempArray) {
